...
| Should `new_group` become a formal Launcher classification? | Open for future packet | V1.1 shows it as a proposal only because the approved classification model still has five types. |
| Should project/module hierarchy come from registry records instead of static local options? | Open for future packet | V1.1 now makes modules visible, but does not solve durable hierarchy modeling. |
